...
# Требования к визуализации: Диаграмма Санкея

## 1. Библиотека визуализации
ECharts

## 2. Тип визуализации
Sankey Diagram (Диаграмма Санкея)

## 3. Желаемый результат

### Визуальные характеристики:
- Узлы (прямоугольники) представляют категории или этапы
- Связи (изогнутые полосы) показывают поток между узлами
- Ширина связей пропорциональна значению потока
- Цвет узлов из палитры Visiology
- Связи наследуют цвет исходного узла с прозрачностью
- Подписи узлов с названиями категорий

### Функциональность:
- При наведении на узел/связь подсвечиваются связанные элементы
- Всплывающая подсказка с детальной информацией о потоке
- Возможность перетаскивания узлов
- Автоматическая компоновка для минимизации пересечений

## 4. Структура данных

### Измерения:
- **Измерение[0]:** Источник потока
- **Измерение[1]:** Назначение потока

### Показатели:
- **Показатель[0]:** Значение потока
